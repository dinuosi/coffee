...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>上架时间</label>
										<div id="shangjiashijianDate" class="input-append date form_datetime">
											<input id="shangjiashijian-input" name="shangjiashijian" size="15" type="text" readonly>
									...
									<div class="form-group col-md-6">
										...
										<input name="alllimittimes" id="alllimittimes" name="alllimittimes" class="form-control" placeholder="库存">
											...
											<div class="form-group  col-md-12" >
												<label>商品描述</label>
												<div><div class="upload"><input id="shangpinmiaoshuupload" name="file" type="file">选择文件</div>
												<script id="shangpinmiaoshuEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var shangpinmiaoshuUe = UE.getEditor('shangpinmiaoshuEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="shangpinmiaoshu" id="shangpinmiaoshu-input">
												</div>
											</div>	
	...
	<script>
		...
		function upload(){
			...
			$('#shangpinmiaoshuupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('shangpinmiaoshuEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.shangpinmiaoshu != null && ruleForm.shangpinmiaoshu != 'null' && ruleForm.shangpinmiaoshu != ''){
				var shangpinmiaoshuMes = '' + ruleForm.shangpinmiaoshu;
				var shangpinmiaoshuUeditor = UE.getEditor('shangpinmiaoshuEditor');
				shangpinmiaoshuUeditor.ready(function() {
					shangpinmiaoshuUeditor.setContent(shangpinmiaoshuMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('shangpinmiaoshuEditor').hasContents()){
				$('#shangpinmiaoshu-input').attr('value',UE.getEditor('shangpinmiaoshuEditor').getContent());
			}
		}
		...
		function dateTimePick(){
			...
			$("#shangjiashijian-input").val(getCurDate());
			$("#shangjiashijianDate").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
		...
		function dataBind(){
			...
			$("#shangjiashijian-input").val(ruleForm.shangjiashijian);
			$("#shangpinmiaoshu").val(ruleForm.shangpinmiaoshu);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.shangjiashijian != null && res.data.shangjiashijian != ''){

						$("#shangjiashijian-input").val(res.data.shangjiashijian);
						$('#shangjiashijianFileName').val(res.data.shangjiashijian)
					}
					if(res.data != null && res.data != '' && res.data.shangpinmiaoshu != null && res.data.shangpinmiaoshu != ''){

						$("#shangpinmiaoshu").val(res.data.shangpinmiaoshu);
                        var shangpinmiaoshuMes = '' + res.data.shangpinmiaoshu;
                        var shangpinmiaoshuUeditor = UE.getEditor('shangpinmiaoshuEditor');
                        shangpinmiaoshuUeditor.ready(function() {
                            shangpinmiaoshuUeditor.setContent(shangpinmiaoshuMes);
                        });
